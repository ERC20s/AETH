...
// import { unos } from './game'
import msgtype from "./msgtype";
import { UNO_BOT } from "./botUno";
import * as utils from "@dcl/ecs-scene-utils";
import { UNO } from "./uno";
import { QuizManager } from "src/quiz-manage/index";
import Parachain from "src/parachains";

// how often the lastKicker player sends updates to server, in seconds
let timeout: Entity;
let botTimeout: Entity;
export let quizManager: QuizManager[] = [];
export let paraInstance: Parachain[] = [];
export let unos: UNO[] = [];
...
export async function joinSocketServer() {
  ...
  socket.onmessage = async function (event) {
    try {
      ...
      if (quizManager.length) {
        const quizUploader = quizManager[0];
        switch (msg.type) {
          case msgtype.QUIZ_GET_REPLY:
            quizUploader.showReviewDialog(msg.data);
            return;
          case msgtype.QUIZ_ACCEPT_REPLY:
          case msgtype.QUIZ_DECLINE_REPLY:
            quizUploader.onWait = false;
            quizUploader.reviewData = msg.data.map((item: any) => {
              return {
                ...item,
            ...
            quizUploader.setQuizToReview();
            return;
        }
      }
      if (paraInstance.length) {
        const parachain = paraInstance[0];
        switch (msg.type) {
          case msgtype.GET_QUIZ_BY_CATEGORY_REPLY:
            parachain.selectCategory(msg.data);
            return;
          case msgtype.GET_HISTORY_REPLY:
            parachain.setHistory(msg.data)
            return;
        }
      }
